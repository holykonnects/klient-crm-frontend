...
function recomputeDraftLive(next) {
  ...
  const useAutoAmount = hasQtyRate ? next.__useAutoAmount !== false : false; // default true when qty/rate exists
  const useManualTotal = !hasQtyRate ? Boolean(next.__useManualTotal) : false; // only allowed when no qty/rate
...
export default function CostingTable() {
  ...
  const costSheetOptions = useMemo(() => {
    ...
    return arr
      .map((cs) => {
        const id = String(cs?.["Cost Sheet ID"] || "").trim();
  ...
  const selectedExtractCostSheetOption = useMemo(() => {
    ...
    if (!id) return null;
    return costSheetOptions.find((x) => x.id === id) || null;
  }, [extractForm?.costSheetId, costSheetOptions]);
  ...
  const selectedAddExpenseCostSheetOption = useMemo(() => {
    ...
    if (!id) return null;
    return costSheetOptions.find((x) => x.id === id) || null;
  }, [selectedExistingSheetId, costSheetOptions]);
  ...
  function openCreateModal() {
    ...
    setCreateForm((p) => ({
      ...p,
      Owner: isAdmin ? (p.Owner || loggedInName || "") : loggedInName || p.Owner,
      Status: "Draft",
      "Linked Entity Type": "",
      "Linked Entity ID": "",
      "Linked Entity Name": "",
  ...
  async function createCostSheet() {
    ...
    try {
      const payload = {
        ...createForm,
        Owner: isAdmin ? (createForm.Owner || loggedInName || "") : loggedInName || createForm.Owner,
      };

      await apiPost({ action: "createCostSheet", data: payload });
      setOpenCreate(false);
  ...
  async function addLineItemRow(row) {
    ...
    payloadRow["Project Type"] = activeSheet?.["Project Type"] || payloadRow["Project Type"] || "";
  ...
  async function softDeleteLineItem(item) {
    ...
    if (!particular && !entryTimestamp) {
      alert("Cannot delete: This row has no Particular and no Entry Timestamp.");
      return;
    }
    ...
    try {
      if (addExpenseMode === "existing") {
        ...
        const payloads = validItems.map((p) => ({
          ...
          "Linked Entity Type": sheetRow?.["Linked Entity Type"] || p["Linked Entity Type"] || "",
          "Linked Entity ID": sheetRow?.["Linked Entity ID"] || p["Linked Entity ID"] || "",
          "Linked Entity Name": sheetRow?.["Linked Entity Name"] || p["Linked Entity Name"] || "",
  ...
  const filteredLineSearchRows = useMemo(() => {
    ...
    return (lineSearchRows || []).filter((row) => {
      if (
        ...
        String(row?.["Linked Entity Type"] || "").trim() !== lineSearchEntityType
      ) {
        return false;
      }
      ...
      if (
        ...
        String(row?.["Payment Status"] || "").trim() !== lineSearchPaymentStatus
      ) {
        return false;
      }

      if (lineSearchHead && String(row?.["Head Name"] || "").trim() !== lineSearchHead) {
        return false;
      }
      ...
      if (lineSearchColumn !== "all") {
        return String(row?.[lineSearchColumn] ?? "").toLowerCase().includes(q);
      }

      return lineSearchAllColumns.some((c) =>
  ...
  const lineSearchTotals = useMemo(() => {
    return filteredLineSearchRows.reduce(
      (acc, row) => ({
        count: acc.count + 1,
        qty: acc.qty + safeNum(row?.["QTY"]),
        amount: acc.amount + safeNum(row?.["Amount"]),
        gst: acc.gst + safeNum(row?.["GST Amount"]),
        total: acc.total + safeNum(row?.["Total Amount"]),
      }),
  ...
  const lineSearchGroupedTotals = useMemo(() => {
    ...
    filteredLineSearchRows.forEach((row) => {
      const rawKey = String(row?.[lineSearchGroupBy] || "").trim();
      ...
      current.count += 1;
      current.qty += safeNum(row?.["QTY"]);
      current.amount += safeNum(row?.["Amount"]);
      current.gst += safeNum(row?.["GST Amount"]);
      current.total += safeNum(row?.["Total Amount"]);
      map.set(key, current);
    });
                ...
                <Paper variant="outlined" sx={{ borderRadius: 2, overflow: "hidden" }}>
                  ...
                  <TableContainer sx={{ maxHeight: 520 }}>
                    <Table size="small" stickyHeader>
                      ...
                      <TableBody>
                        {filteredLineSearchRows.map((row, idx) => (
                          <TableRow key={row.__lineKey || idx} hover>
                            ...
                            <TableCell sx={{ whiteSpace: "nowrap" }}>
                              <IconButton
                                ...
                                onClick={() => openEditModalById(row?.["Cost Sheet ID"])}
                              >
            ...
            <Grid container spacing={1.5} sx={{ mt: 0.5 }}>
              ...
              <Grid item xs={12}>
                <Autocomplete
                  ...
                  value={selectedExtractCostSheetOption}
                ...
                <>
                  <Grid item xs={12} md={6}>
                    <TextField
                      ...
                      helperText={!isAdmin ? "Owner is locked to the logged-in user." : ""}
                    />
                  </Grid>
                        ...
                        >
                          ...
                          >
                            <RemoveCircleOutlineIcon
                              sx={{ color: (addExpenseItems || []).length <= 1 ? "#bbb" : "#c62828" }}
                            />
                          </IconButton>
                        </Box>
                        ...
                        <Grid container spacing={1.25}>
                            ...
                            <FormControlLabel
                              control={
                                <Checkbox
                                  ...
                                  disabled={!hasQtyRate}
                                  onChange={(e) => setAddExpenseItemField(idx, "__useAutoAmount", e.target.checked)}
                                />
                              }
                          ...
                          <Grid item xs={12}>
                            <TextField
                              ...
                              InputProps={{ readOnly: Boolean(it?.__useAutoAmount) && hasQtyRate }}
                              helperText={
                                hasQtyRate
                          ...
                          <Grid item xs={6}>
                            <TextField
                              ...
                              InputProps={{ readOnly: !canManualTotal }}
                              helperText={
                                canManualTotal
          ...
          <DialogContent dividers sx={{ position: "relative" }}>
            ...
            >
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 1.25 }}>
                ...
                <Grid container spacing={1}>
                  ...
                  <Grid item xs={6}>
                    <TextField
                      ...
                      InputProps={{ readOnly: Boolean(drawerDraft.__hasQtyRate) }}
                      helperText={
                        drawerDraft.__hasQtyRate
                          ? "Auto (Amount + GST Amount)"
                          : "Manual Total (when QTY×Rate is unavailable)"
                ...
                {drawerMode === "edit" ? (
                  <Typography sx={{ fontSize: 11, opacity: 0.75 }}>
                    Note: Edit marks the old row inactive and appends the updated row (audit-safe). The UI removes the
                    old row using a stable key to avoid duplicates.
                  </Typography>
                ) : null}
              </Box>
            </Drawer>
